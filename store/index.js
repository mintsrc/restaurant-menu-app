...
const mutations = {
  ...
  EDIT_CATEGORY(state, payload) {
    _.assign(
      state.menu.metadata.menu[state.selectedCategory],
      _.cloneDeep(payload)
    );
  },
...
const actions = {
  getMenus(context) {
    ...
    Request.getMenus(this.state.selectedDate.toISOString())
      .then(res => {
        console.log("Response: ", !isEmpty(res));
        if (!isEmpty(res)) {
          res.objects.all[0].metadata.menu = JSON.parse(
            res.objects.all[0].metadata.menu
          );
          context.commit("SET_MENUS", res.objects.all[0]);
          context.commit("SET_IS_PRESENT", false);
        } else {
          Request.getMenus("default")
            .then(res => {
  ...
  addMenu(context, payload) {
    context.commit("LOADING");
    Request.getMenus(context.getters.menu.metadata.date.toISOString())
      .then(res => {
        if (isEmpty(res)) {
          Request.addMenu(payload)
            .then(menu => {
  ...
  editMenuItem(context, payload) {
    context.commit("MENU_ITEM_LOADING");
    if (
      isValueUnique(context.getters.category.menuItems, payload, "title", true)
    ) {
      Request.saveMedia(payload)
        .then(payload => {
  ...
  deleteMenuItem(context, payload) {
    context.commit("MENU_ITEM_LOADING");
    Request.deleteMedia(state.category.menuItems[payload].feature_image.id)
      .then(res => {
  ...
  editCategory(context, payload) {
    ...
    if (
      isValueUnique(context.getters.menu.metadata.menu, payload, "title", true)
    ) {
      delete payload.index;
